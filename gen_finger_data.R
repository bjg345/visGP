...
for(i in 1:250){
  for(j in 1:100){
      A[((i-1)*100+j), 1:((i-1)*100+j)] = readRDS(file.path('A_finger', paste0('Avec_', i, '.rds')))[[j]]
    }
}
A[upper.tri(A)] = t(A)[upper.tri(A)]
diag(A) = 0
saveRDS(A, 'finger_u.rds')

grid = points@coords; rownames(grid)=c()

ground = as.vector(sapply(1:250, function(i) readRDS(file.path('ground_finger', paste0('ground_', i, '.rds')))))
