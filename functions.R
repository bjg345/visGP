...
cov.select.loglik = function(param, nu = nu, y, dists, cliques,
  ...
  if(!is.null(n.neighbors)){
    ...
    out = -sum(sapply(cliques, clique.lik)) + sum(sapply(seps, clique.lik))
  ...
  if(method == 'cov.select'){
    ...
    if(!is.null(n.cliques)){
        if(n.cliques < length(cliques)){
          clique.lengths = sapply(cliques, length)
          cl.ord = order(clique.lengths, decreasing=T)
        	#clique.ind = sample.int(length(cliques), n.cliques)
        	clique.ind = cl.ord[1:n.cliques]
    ...
    if(length(y) > 2500) {fit = sgd(est.start= c(log(c(range.start, nugget.start, sigma.start)), mu.start),
     y.clique = lapply(cliques, function(c) y[c]), y.sep = lapply(seps, function(s) y[s]), 
     D.clique = lapply(cliques, function(c) D[c,c]), D.sep = lapply(seps, function(s) D[s,s]), 
    ...
    fit = optim(par = log(c(range.start,nugget.start,sigma.start)), fn = neighbor.loglik, nu = nu, neighbor.list = neighbor.list, y = y, dists = D.adj, coords = coords,
                 ord = ord)          
...
grad = function(y, covpar, mu, D){
  ...
  Sigma=Sigma.fun(covpar, D)
  W = Matrix::chol2inv(Matrix::chol(Sigma))
  dS = dSigma.fun(covpar,D)
  dW = lapply(dS, function(x) -W%*%x%*%W)
  ydiff = y-mu
  grad.cov = sapply(1:3, function(i) -1/2*( t(ydiff) %*% dW[[i]] %*% (ydiff))-
                       1/2*sum( diag(W %*% dS[[i]]) ) )
  grad.mu = sum(W %*% (ydiff))
  return(c(grad.cov, grad.mu))
}

sgd = function(est.start, y.clique, y.sep, D.clique, D.sep, alpha = .025, beta = .99,  epsilon = 1e-9, count.max = 5000){
  m = length(y.clique)
  est.cur = est.start
  v_t <- numeric(4)
  count = 0
  repeat {
    ...
    for (i in ord) {
      count = count+1
      g <- grad(y.clique[[i]], est.cur[1:3], est.cur[4], D.clique[[i]]) -
        grad(y.sep[[i]], est.cur[1:3], est.cur[4], D.sep[[i]])
      
      v_t <- beta * v_t + (1 - beta) * (g * g)       # Update second moment estimate
      step <- alpha * g / (sqrt(v_t) + epsilon)      # Update step
      est.new <- est.cur + step
      
      print(c(exp(c(est.new)[1:3]), est.new[4]))
      est.cur <- est.new
      if (count >= count.max) break
      
    }
    return(list(par=est.cur))
  }
