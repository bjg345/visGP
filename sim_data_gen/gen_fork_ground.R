# Generate 'ground-truth' function values for U-shaped domain

id = as.numeric(commandArgs(trailingOnly = T))

if(file.exists(file.path('ground_fork', paste0('ground_', id, '.rds')))) quit()
...
if(!dir.exists('ground_fork')) dir.create('ground_fork')
...
saveRDS(ground, file.path('ground_fork', paste0('ground_', id, '.rds')))
