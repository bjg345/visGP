# Calculate adjacency relations for fork-shaped domain

id = as.numeric(commandArgs(trailingOnly = T))

if(!dir.exists('A_fork')) dir.create('A_fork')
if(file.exists( file.path('A_fork', paste0('Avec_', id, '.rds')) )) quit()
...
saveRDS(out.list, file.path('A_fork', paste0('Avec_', id, '.rds')))
