# Calculate adjacency relations for U-shaped domain

id = as.numeric(commandArgs(trailingOnly = T))

if(!dir.exists('A_u')) dir.create('A_u')
if(file.exists( file.path('A_u', paste0('Avec_', id, '.rds')) )) quit()
...
saveRDS(out.list, file.path('A_u', paste0('Avec_', id, '.rds')))
