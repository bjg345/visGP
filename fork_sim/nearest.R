...
nearest.fit = readRDS(file.path('visgp_fit', paste0('fit_', n, '_', stdev, '_', id, '.rds')))

nearest.pred = sapply(1:nrow(grid.test), function(i) pred_neighbor(new_loc = grid.test[i,], A=A.train, A_vec = A.test[i, ], 
                                          grid.train, vals.train, nearest.fit$par, nu=.5, method = 'nearest.clique', n_neighbors = 10, 
                                           D=D.train, water) )
