...
if(!dir.exists('bora')) dir.create('bora')
...
upper = 10*phi
if (upper <= 1e-6) upper = 1 # fix plugged in for one problematic sim run
priors <- list("phi.Unif" = c(1e-6, upper),                              
...
barrier_nninfo_all <- barrier_neighbor(coords = grid.train, coords.0 =  grid.test, ord = ord,
                                                n.neighbors = m,
                                                barrier = st_as_sf(bound), min.gridsize = .01,
                                                cores = 1,
                                                verbose = T,
