...
maxprec.fit = readRDS(file.path('cs_fit', paste0('fit_', n, '_', stdev, '_', id, '.rds')))

maxprec.pred = sapply(1:nrow(grid.test), function(i) pred_neighbor(new.loc = grid.test[i,], A=A.train, A.vec = A.test[i, ], 
                                          grid.train, vals.train, maxprec.fit$par, nu=.5, method = 'maxprec', n.neighbors = 10, 
                                           D=D.train, water) )
