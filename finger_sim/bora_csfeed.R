...
if(!dir.exists('bora_csfeed')) dir.create('bora_csfeed')
...
if(file.exists( file.path('bora_csfeed', paste0('vals_', n, '_', stdev, '_', id, '.rds')) )) quit()
...
nu <- 0.5
cs.fit = readRDS(file.path('cs_fit',  paste0('fit_', n, '_', stdev, '_', id, '.rds')))
phi = exp(-cs.fit$par[1])
tau.sq = exp(cs.fit$par[2])
sigma.sq = exp(cs.fit$par[3]*2)
beta = cs.fit$par[4]
starting <- list("phi" = phi, "sigma.sq" = sigma.sq, 
                 "tau.sq" = tau.sq, "nu" = nu, "beta"=beta)
tuning <- list("phi" = 0, "sigma.sq" = 0, "tau.sq" = 0.1, "nu" = 0)         # nu is fixed 
...
barrier_nn.indx.0_list <- barrier_nninfo_all$barrier_nn.indx.0_list
      ...
      barrier_m.s <- spNNGP(vals.train~ 1, coords = grid.train, starting = starting,
       ...
       saveRDS(barrier_m.s, file.path('bora_csfeed', paste0('fit_', n, '_', stdev, '_', id, '.rds')))
        saveRDS(list(pred=ystarBRGP, ci = yquantBRGP), file.path('bora_csfeed', paste0('pred_', n, '_', stdev, '_', id, '.rds')))
          saveRDS(ystarBRGP-vals.test, file.path('bora_csfeed', paste0('err_', n, '_', stdev, '_', id, '.rds')))
      saveRDS(vals.test, file.path('bora_csfeed', paste0('vals_', n, '_', stdev, '_', id, '.rds')))
